...
um.upsilon_t_s = pyo.Var() # LMP of carbon / Carbon allowance trades in the wholesale market
...
um.objective = pyo.Objective(
    expr=sum(
        um.p_s_t * (
            ...
            um.um.upsilon_t_s[t, s] * um.rho2_SE_t_s[t, s] +
            # Receita de intercâmbio de energia com o ISO nos nós de interface
            sum(
                um.P_SE_i_t_s[i, t, s] * um.lambda_t_s[t, s]    
                for i in um.N_INF
            )
        )
        for s in s 
